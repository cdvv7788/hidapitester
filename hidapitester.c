...
static void print_usage(char *myname)
{
    fprintf(stderr,
            ...
            "  --wait <msecs>              Wait for msecs milliseconds \n"
...
void printbuf(uint8_t *buf, int bufsize, int base, int width)
{
    for (int i = 0; i < bufsize; i++)
    {
        ...
        if (i % width == width - 1 && i < bufsize - 1)
            printf("\n");
    }
    printf("\n");
}
    ...
    struct option longoptions[] =
        {
            ...
            {"wait", required_argument, &cmd, CMD_WAIT},
    ...
    while (!done)
    {
        ...
        case 0: // long opts with no short opts
            ...
            if (cmd == CMD_VIDPID)
            {
                ...
                if (sscanf(optarg, "%4hx/%4hx", &vid, &pid) != 2)
                { // match "23FE/AB12"
                    if (!sscanf(optarg, "%4hx:%4hx", &vid, &pid))
                    { // match "23FE:AB12"
            ...
            else if (cmd == CMD_USAGEPAGE)
            {
                ...
                if ((usage_page = strtol(optarg, NULL, 0)) == 0)
                {                                        // if bad parse
            ...
            else if (cmd == CMD_USAGE)
            {
                ...
                if ((usage = strtol(optarg, NULL, 0)) == 0)
                {                                   // if bad parse
            ...
            {
                int wait_millis = strtol(optarg, NULL, 10);
                msg("Waiting %d milliseconds...\n", wait_millis);
                Sleep(wait_millis);
            }
